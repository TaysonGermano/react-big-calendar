...
var EventItem =
    ...
    (function (_Component) {
      ...
      function EventItem(props) {
        ...
        var _this = _possibleConstructorReturn(
          this,
          (EventItem.__proto__ || Object.getPrototypeOf(EventItem)).call(
            this,
            props
          )
        );
        ...
        },
        {
          ...
          value: function render() {
            ...
            if (this.endResizable(this.props))
              endResizeDiv = _react2.default.createElement("div", {
  ...
  (_initialiseProps = function _initialiseProps() {
    ...
    this.initStartDrag = function (ev) {
      ...
      if (supportTouch) {
        _this3.startResizer.addEventListener(
          "touchmove",
          _this3.doStartDrag,
          false
        );
        _this3.startResizer.addEventListener(
          "touchend",
          _this3.stopStartDrag,
          false
        );
        _this3.startResizer.addEventListener(
          "touchcancel",
          _this3.cancelStartDrag,
          false
        );
      } else {
    ...
    this.stopStartDrag = function (ev) {
      ev.stopPropagation();
      if (supportTouch) {
        _this3.startResizer.removeEventListener(
          "touchmove",
          _this3.doStartDrag,
          false
        );
      ...
      var newStart = localeMoment(eventItem.start)
        .add(
          cellUnit === _index.CellUnits.Hour
            ? count * config.minuteStep
            : count,
          cellUnit === _index.CellUnits.Hour ? "minutes" : "days"
        )
        .format(_index.DATETIME_FORMAT);
      if (
        count !== 0 &&
        cellUnit !== _index.CellUnits.Hour &&
    ...
    this.initEndDrag = function (ev) {
      ...
      schedulerData._startResizing();
      if (supportTouch) {
        _this3.endResizer.addEventListener(
          "touchmove",
          _this3.doEndDrag,
          false
        );
    ...
    this.stopEndDrag = function (ev) {
      ...
      if (supportTouch) {
        _this3.endResizer.removeEventListener(
          "touchmove",
          _this3.doEndDrag,
          false
        );
      ...
      var newEnd = localeMoment(eventItem.end)
        .add(
          cellUnit === _index.CellUnits.Hour
            ? count * config.minuteStep
            : count,
    ...
    this.endResizable = function (props) {
      ...
      return (
        ...
        isInPopover === false &&
        (eventItem.resizable == undefined || eventItem.resizable !== false) &&
        (eventItem.endResizable == undefined ||
          eventItem.endResizable !== false)
      );
    };
    ...
    this.subscribeResizeEvent = function (props) {
      if (_this3.startResizer != undefined) {
        if (supportTouch) {
          ...
          _this3.startResizer.removeEventListener(
            "mousedown",
            _this3.initStartDrag,
            false
          );
          if (_this3.startResizable(props))
            _this3.startResizer.addEventListener(
              "mousedown",
              _this3.initStartDrag,
              false
